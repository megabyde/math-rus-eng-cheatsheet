...
\begin{enumerate}
  ...
  \item Используйте в переводе следующие сокpaщения: \textsf{
  \begin{center}
    \begin{tabular}{lll}
  ...
  \begin{enumerate}   
    \item Для того, чтобы подчеркнуть, что данное действие было выполнено этим, а не каким-либо другим aгeнтом, например:\\ [4pt] \textsf{
    ...
    It is this phenomenon that accounts for such a behaviuor of the system.}
    ...
    This effect is accouted for by two reasons, namely large temperature gradient and a poor heat conductivity of the walls.\\ [4pt]}
  ...
  \begin{enumerate}   
    \item Помните имена существительные, которые формально имеют множественное число, но должны употребляться со сказуемым в единственном числе, если речь идёт о данных объектах, взятых и совокупности, а не по-отдельности: \textsf{
    \begin{center}
      \begin{tabular}{lllll}
      knoweledge & news     & money  & information & contents\\
    ...
    \item Единицы измерения считаются собирательными существительными и употребляются со сказуемом в единственном числе, например:\\ [4pt] \textsf{
    ...
    \item Составное подлежащее, содержащее слова <<each>>, <<every>>, могут  употребляться со сказуемом в единственном числе, например:\\ [4pt] \textsf{
  ...
  \begin{enumerate}   
    ...
    \begin{center}
      ...
      \end{tabular}
    \end{center}}   
    Ho: <<pre-Newtonion era>> или <<non-Gaussian distribution>>
    ...
    \item Используйте дефис в модификаторах, которые содержат глаголы или глагольные формы, например: <<laser-induсed reaction>>,\\ <<problem-solving abilities>>, <<immobilized-phase method>>.
  ...
  \item Не путайте <<it's>> и <<its>>. <<It's>> "--- это краткая форма от <<it is>>, a <<its>> "--- это <<его, её>> по отношению к неодушевлённым предметам.
...
\dictentry{гиперболоид} {hyperboloid;\\
  \textbf{усечённый однолистный \word\ вращения} truncated one-sheeted hyperboloid of revolution;
}
...
\dictentry{преходить} {change [over] to, switch [over] to, transform to;\\
